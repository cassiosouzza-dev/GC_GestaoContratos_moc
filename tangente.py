...
import urllib.request  # Adicionado pois é usado no updater
...
def obter_chave_api():
    ...
    # Tenta os dois nomes possíveis por segurança
    opcoes = ["chave_api.txt", "chave_api.txt.txt"]

    for nome in opcoes:
        arquivo_chave = os.path.join(caminho_base, nome)
        if os.path.exists(arquivo_chave):
            try:
                with open(arquivo_chave, "r", encoding="utf-8") as f:
                    chave = f.read().strip()
                    if len(chave) > 10: return chave
            except:
                pass
    return None
...
class DialogoPagamento(BaseDialog):
    ...
    def __init__(self, comp_inicio, comp_fim, pg_editar=None, titulo="Realizar Pagamento", label_valor="Valor:",
        ...
        # --- CORREÇÃO VISUAL: Força estilo limpo na lista para as caixas aparecerem ---
        self.lista_comp.setStyleSheet("""
            ...
            QListWidget::item:hover {
                background-color: #eafaf1; /* Verde bem clarinho ao passar mouse */
            }
            QListWidget::item:selected {
                background-color: #d5f5e3;
                color: #000000;
            }
            /* Estilo dos Quadradinhos (Checkbox) */
            QListWidget::indicator {
                width: 18px;
                height: 18px;
                border: 2px solid #7f8c8d; /* Borda cinza escuro visível */
                background-color: #ffffff;
                border-radius: 3px;
            }
            QListWidget::indicator:checked {
                background-color: #27ae60; /* Fundo verde quando marcado */
                border-color: #27ae60;
            }
            QListWidget::indicator:unchecked:hover {
                border-color: #27ae60; /* Borda verde ao passar mouse */
            }
        """)
        # ---------------------------------------------------------------------------
...
class DialogoGerenciarPrestadores(BaseDialog):
    ...
    def excluir_prestador(self):
        ...
        if DarkMessageBox.question(self, "Excluir",
                                   f"Tem certeza que deseja excluir '{p.nome_fantasia}'?") == QMessageBox.StandardButton.Yes:
...
class SistemaGestao(QMainWindow):
    ...
    def sincronizar_nuvem(self):
        ...
        # Texto de status dependendo se achou ou não
        msg_status = "✅ Arquivo encontrado na nuvem!" if arquivo_remoto else "⚠️ Não existe arquivo na nuvem ainda."

        # --- MENU DE OPÇÕES DETALHADO (Lógica Tangente) ---
        mbox = DarkMessageBox(self)
        mbox.setWindowTitle("Sincronização Nuvem (Enterprise)")

        # Texto explicativo rico
        mbox.setText(f"STATUS: {msg_status}\n\nEscolha a operação desejada:")

        # Botão 1: Sincronização Inteligente (O padrão ouro)
        btn_sync = mbox.addButton("⬇️⬆️ Sincronizar Tudo\n(Mesclar Equipe)", QMessageBox.ButtonRole.ActionRole)

        # Botão 2: Upload Seguro (Só sobe o seu, não baixa nada)
        btn_subir_safe = mbox.addButton("⬆️ Apenas Subir\nMinhas Alterações", QMessageBox.ButtonRole.ActionRole)

        # Botão 3: Download Separado (Recuperado da versão de casa)
        btn_baixar_sep = mbox.addButton("⬇️ Baixar Cópia da Nuvem\n(Salvar Como...)", QMessageBox.ButtonRole.ActionRole)

        # Botão 4: Reset (Perigoso)
        btn_reset = mbox.addButton("⚠️ Sobrescrever Nuvem\n(Reset Total)", QMessageBox.ButtonRole.ActionRole)

        mbox.addButton("Cancelar", QMessageBox.ButtonRole.RejectRole)

        mbox.exec()

        # Se não tem arquivo na nuvem e o usuário não cancelou, força criação (Reset/Upload)
        if not arquivo_remoto and mbox.clickedButton() != mbox.button(QMessageBox.StandardButton.Cancel):
            if mbox.clickedButton() == btn_baixar_sep:
                DarkMessageBox.warning(self, "Erro", "Não há nada na nuvem para baixar.")
                return
        ...
        if mbox.clickedButton() == btn_sync:
            # Baixa, compara, mescla e sobe de volta
            self._executar_sincronizacao_inteligente(driver, arquivo_remoto['id'], nome_nuvem)

        elif mbox.clickedButton() == btn_subir_safe:
            # Pega o que tem lá, joga o seu por cima e salva (sem mudar o seu local)
            self._executar_upload_uniao_sem_baixar(driver, arquivo_remoto['id'], nome_nuvem)

        elif mbox.clickedButton() == btn_baixar_sep:
            # Baixa o arquivo para uma pasta local sem substituir o banco atual
            self._executar_download_separado(driver, arquivo_remoto['id'])

        elif mbox.clickedButton() == btn_reset:
            # Apaga o da nuvem e coloca o seu (Perigo!)
            if DarkMessageBox.question(self, "Cuidado Extremo!",
                                       "Isso vai APAGAR tudo o que seus colegas fizeram na nuvem e colocar APENAS os dados que estão neste computador.\n\n"
                                       "Tem certeza absoluta que deseja fazer isso?") == QMessageBox.StandardButton.Yes:
    ...
    def init_ui(self):
        ...
        self.tabela_resultados.setColumnWidth(4, 80)
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.Fixed);
        self.tabela_resultados.setColumnWidth(5, 80)
        header.setSectionResizeMode(6, QHeaderView.ResizeMode.Stretch)
        header.setSectionResizeMode(7, QHeaderView.ResizeMode.Fixed);
        self.tabela_resultados.setColumnWidth(7, 90)
